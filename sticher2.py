...
if __name__ == "__main__":
    ...
    cv2.imwrite("left-top-down.jpeg", left_top_down)
    cv2.imwrite("right-top-down.jpeg", right_top_down)

    # --- 4. Combine into bird's-eye view ---
    ...
    # Place FRONT image
    front_resized = cv2.resize(front_top_down, (car_area_width, h))
    birds_eye_view[0:h, x_offset:x_offset + car_area_width] = front_resized

    # Place BACK image (flipped 180°)
    back_flipped = cv2.rotate(back_top_down, cv2.ROTATE_180)
    back_resized = cv2.resize(back_flipped, (car_area_width, h))
    birds_eye_view[y_offset + car_area_height:y_offset + car_area_height + h,
                   x_offset:x_offset + car_area_width] = back_resized

    # Place LEFT image (rotated)
